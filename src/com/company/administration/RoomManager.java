...
public class RoomManager {
     ...
     */
    public <S extends Room> void bookRoom(RoomAccess access, Human client,
                                          Class<S> type, int peopleCount) {
        Room currentRoom;
        for (int i = 0; i < access.getRooms().size(); i++) {
            currentRoom = access.getRooms().get(i);
            if (!currentRoom.isOccupied() && currentRoom.getSize() >= peopleCount && currentRoom.getClass() == type) {
